...
+ (instancetype)indexPathWithCol:(NSInteger)col row:(NSInteger)row {
    G100IndexPath *indexPath = [[self alloc] initWithColumn:col row:row];
    return indexPath;
}
...
@implementation G100BackgroundCellView
...
@interface G100DropDownMenu (){
    struct {
        unsigned int numberOfRowsInColumn :1;
        unsigned int numberOfItemsInRow :1;
        unsigned int menuColomnStyleForColumn :1;
        unsigned int titleForMenuInColumn :1;
        ...
        unsigned int detailButtonForRowAtIndexPath: 1;
...
@property (nonatomic, strong) UIView *buttonView;   // 放置buttonView
@property (nonatomic, strong) UIButton *resetButton;  // 重置筛选按钮
@property (nonatomic, strong) UIButton *confirmButton;  // 确定筛选按钮
...
#define kButtonPadding 20
#define kButtonHeight 40
#define kButtonWidth 100
#define kButtonViewHeight 80
#define kButtonTitleSize 18
...
@implementation G100DropDownMenu {
...
{
    [self selectIndexPath:[G100IndexPath indexPathWithCol:0 row:0]];
}

- (void)selectIndexPath:(G100IndexPath *)indexPath triggerDelegate:(BOOL)trigger {
    ...
    if (indexPath.item < 0 ) {
        if (!_isClickHaveItemValid && [_dataSource menu:self numberOfItemsInRow:indexPath.row column:indexPath.column] > 0){
            title.string = [_dataSource menu:self titleForItemsInRowAtIndexPath:[G100IndexPath indexPathWithCol:indexPath.column row:self.isRemainMenuTitle ? 0 : indexPath.row item:0]];
            if (trigger) {
                [_delegate menu:self didSelectRowAtIndexPath:[G100IndexPath indexPathWithCol:indexPath.column row:indexPath.row item:0]];
            }
        }else {
            title.string = [_dataSource menu:self titleForRowAtIndexPath:
                            [G100IndexPath indexPathWithCol:indexPath.column row:self.isRemainMenuTitle ? 0 : indexPath.row]];
            if (trigger) {
...
- (void)selectIndexPath:(G100IndexPath *)indexPath {
    [self selectIndexPath:indexPath triggerDelegate:YES];
}

#pragma mark - setter
- (void)setDataSource:(id<G100DropDownMenuDataSource>)dataSource {
    ...
    _dataSourceFlags.menuColomnStyleForColumn = [_dataSource respondsToSelector:@selector(menu:colomnStyleForColumn:)];
    _dataSourceFlags.titleForMenuInColumn = [_dataSource respondsToSelector:@selector(menu:titleForColumn:)];
    ...
    _dataSourceFlags.detailButtonForRowAtIndexPath = [_dataSource respondsToSelector:@selector(menu:detailButtonForRowAtIndexPath:)];
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        if (!self.isClickHaveItemValid && _dataSourceFlags.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:0 column:i]>0) {
            titleString = [_dataSource menu:self titleForItemsInRowAtIndexPath:[G100IndexPath indexPathWithCol:i row:0 item:0]];
        }else {
            
            if (_dataSourceFlags.titleForMenuInColumn) {
                titleString = [_dataSource menu:self titleForColumn:i];
            }
            
            if (!titleString) {
                titleString =[_dataSource menu:self titleForRowAtIndexPath:[G100IndexPath indexPathWithCol:i row:0]];
            }
        }
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    if (self) {
        ...
        // buttonView
        _buttonView = [[UIView alloc] initWithFrame:CGRectMake(origin.x, self.frame.origin.y + self.frame.size.height, self.frame.size.width, kButtonViewHeight)];
        _buttonView.backgroundColor = kBgColor;
        // button 重置
        _resetButton = [UIButton buttonWithType:UIButtonTypeSystem];
        _resetButton.frame = CGRectMake(kButtonPadding, kButtonPadding, kButtonWidth, kButtonHeight);
        _resetButton.clipsToBounds = YES;
        _resetButton.layer.cornerRadius = 6.0f;
        [_resetButton.titleLabel setFont:[UIFont systemFontOfSize:kButtonTitleSize]];
        [_resetButton setTitle:@"重置" forState:UIControlStateNormal];
        [_resetButton setTitleColor:[UIColor colorWithRed:102.0/255.0 green:102.0/255.0 blue:102.0/255.0 alpha:1.0] forState:UIControlStateNormal];
        [_resetButton setBackgroundColor:[UIColor colorWithRed:239.0/255.0 green:239.0/255.0 blue:239.0/255.0 alpha:1.0]];
        [_resetButton addTarget:self action:@selector(resetFilter) forControlEvents:UIControlEventTouchUpInside];
        [_buttonView addSubview:_resetButton];
        
        // button 确定
        _confirmButton = [UIButton buttonWithType:UIButtonTypeSystem];
        _confirmButton.frame = CGRectMake(_buttonView.frame.size.width - kButtonPadding - kButtonWidth, kButtonPadding, kButtonWidth, kButtonHeight);
        _confirmButton.clipsToBounds = YES;
        _confirmButton.layer.cornerRadius = 6.0f;
        [_confirmButton.titleLabel setFont:[UIFont systemFontOfSize:kButtonTitleSize]];
        [_confirmButton setTitle:@"确定" forState:UIControlStateNormal];
        [_confirmButton setTitleColor:[UIColor colorWithRed:255.0/25.0 green:255.0/255.0 blue:255.0/255.0 alpha:1.0] forState:UIControlStateNormal];
        [_confirmButton setBackgroundColor:[UIColor colorWithRed:224.0/255.0 green:102.0/255.0 blue:102.0/255.0 alpha:1.0]];
        [_confirmButton addTarget:self action:@selector(confirmFilter) forControlEvents:UIControlEventTouchUpInside];
...
- (void)animateTableView:(UITableView *)tableView show:(BOOL)show complete:(void(^)())complete {
    ...
    BOOL hasButton = NO;
    if (_dataSourceFlags.menuColomnStyleForColumn) {
        if ([_dataSource menu:self colomnStyleForColumn:_currentSelectedMenudIndex] == G100DropDownMenuColomnStyleDefault) {
            hasButton = NO;
        }else if ([_dataSource menu:self colomnStyleForColumn:_currentSelectedMenudIndex] == G100DropDownMenuColomnStyleMultiple) {
            hasButton = YES;
        }
    }
    ...
    if (show) {
        if (haveItems) {
            ...
            if (hasButton) {
                _leftTableView.tableFooterView = _buttonView;
            }else {
                _leftTableView.tableFooterView = nil;
            }
        }
        ...
        if (hasButton) {
            tableViewHeight += kButtonViewHeight;
        }
...
{
    NSString *identifier = [NSString stringWithFormat:@"DropDownMenuCell-%@", @(_currentSelectedMenudIndex)];
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (!cell) {
        cell = [[UITableViewCell alloc] initWithStyle:_cellStyle reuseIdentifier:identifier];
        //cell.separatorInset = UIEdgeInsetsZero;
        G100BackgroundCellView *bg = [[G100BackgroundCellView alloc]init];
    ...
    if (tableView == _leftTableView) {
        if (_dataSourceFlags.titleForRowAtIndexPath) {
            cell.textLabel.text = [_dataSource menu:self titleForRowAtIndexPath:[G100IndexPath indexPathWithCol:_currentSelectedMenudIndex row:indexPath.row]];
            if (_dataSourceFlags.imageNameForRowAtIndexPath) {
                NSString *imageName = [_dataSource menu:self imageNameForRowAtIndexPath:[G100IndexPath indexPathWithCol:_currentSelectedMenudIndex row:indexPath.row]];
            ...
            if (_dataSourceFlags.detailTextForRowAtIndexPath) {
                NSString *detailText = [_dataSource menu:self detailTextForRowAtIndexPath:[G100IndexPath indexPathWithCol:_currentSelectedMenudIndex row:indexPath.row]];
        ...
        if (_dataSourceFlags.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:indexPath.row column:_currentSelectedMenudIndex]> 0 && _dataSourceFlags.menuColomnStyleForColumn && [_dataSource menu:self colomnStyleForColumn:_currentSelectedMenudIndex] == G100DropDownMenuColomnStyleDefault){
        ...
        } else if (_dataSourceFlags.menuColomnStyleForColumn && [_dataSource menu:self colomnStyleForColumn:_currentSelectedMenudIndex] == G100DropDownMenuColomnStyleMultiple) {
            UIButton * button = [UIButton buttonWithType:UIButtonTypeCustom];
            button.frame = CGRectMake(0, 0, 20, 20);
            button.userInteractionEnabled = NO;
            [button setImage:[UIImage imageNamed:@"checkbox1_unchecked"] forState:UIControlStateNormal];
            [button setImage:[UIImage imageNamed:@"checkbox1_checked"] forState:UIControlStateSelected];
            
            if (_dataSourceFlags.detailButtonForRowAtIndexPath && [_dataSource menu:self detailButtonForRowAtIndexPath:[G100IndexPath indexPathWithCol:_currentSelectedMenudIndex row:indexPath.row]]) {
                button.selected = YES;
            }else {
                button.selected = NO;
            }
        ...
        if (_dataSourceFlags.titleForItemsInRowAtIndexPath) {
            NSInteger currentSelectedMenudRow = [_currentSelectRowArray[_currentSelectedMenudIndex] integerValue];
            cell.textLabel.text = [_dataSource menu:self titleForItemsInRowAtIndexPath:[G100IndexPath indexPathWithCol:_currentSelectedMenudIndex row:currentSelectedMenudRow item:indexPath.row]];
            
            if (_dataSourceFlags.imageNameForItemsInRowAtIndexPath) {
                NSString *imageName = [_dataSource menu:self imageNameForItemsInRowAtIndexPath:[G100IndexPath indexPathWithCol:_currentSelectedMenudIndex row:currentSelectedMenudRow item:indexPath.row]];
            ...
            if (_dataSourceFlags.detailTextForItemsInRowAtIndexPath) {
                NSString *detailText = [_dataSource menu:self detailTextForItemsInRowAtIndexPath:[G100IndexPath indexPathWithCol:_currentSelectedMenudIndex row:currentSelectedMenudRow item:indexPath.row]];
                cell.detailTextLabel.text = detailText;
            }else {
                cell.detailTextLabel.text = nil;
            }
...
- (NSIndexPath *)tableView:(UITableView *)tableView willSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.delegate && [_delegate respondsToSelector:@selector(menu:willSelectRowAtIndexPath:)]) {
        return [self.delegate menu:self willSelectRowAtIndexPath:[G100IndexPath indexPathWithCol:_currentSelectedMenudIndex row:indexPath.row]];
    } else {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_leftTableView == tableView) {
        ...
        if (isClickHaveItemValid && _delegate && [_delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
            [self.delegate menu:self didSelectRowAtIndexPath:[G100IndexPath indexPathWithCol:_currentSelectedMenudIndex row:indexPath.row]];
        } else {
            //TODO: delegate is nil
        }
    } else {
        ...
        if (self.delegate && [_delegate respondsToSelector:@selector(menu:didSelectRowAtIndexPath:)]) {
            NSInteger currentSelectedMenudRow = [_currentSelectRowArray[_currentSelectedMenudIndex] integerValue];
            [self.delegate menu:self didSelectRowAtIndexPath:[G100IndexPath indexPathWithCol:_currentSelectedMenudIndex row:currentSelectedMenudRow item:indexPath.row]];
        } else {
            //TODO: delegate is nil
...
- (BOOL )confiMenuWithSelectRow:(NSInteger)row {
    ...
    if (_dataSourceFlags.numberOfItemsInRow && [_dataSource menu:self numberOfItemsInRow:row column:_currentSelectedMenudIndex]> 0) {
        ...
        if (self.isClickHaveItemValid) {
            title.string = [_dataSource menu:self titleForRowAtIndexPath:[G100IndexPath indexPathWithCol:_currentSelectedMenudIndex row:row]];
        ...
        if (!title.string) {
            title.string = [_dataSource menu:self titleForRowAtIndexPath:
                            [G100IndexPath indexPathWithCol:_currentSelectedMenudIndex row:self.isRemainMenuTitle ? 0 : row]];
        }
        
        if (_dataSourceFlags.menuColomnStyleForColumn && [_dataSource menu:self colomnStyleForColumn:_currentSelectedMenudIndex] == G100DropDownMenuColomnStyleDefault) {
            [self animateIdicator:_indicators[_currentSelectedMenudIndex] background:_backGroundView tableView:_leftTableView title:_titles[_currentSelectedMenudIndex] forward:NO complecte:^{
                _show = NO;
            }];
...
- (void)confiMenuWithSelectItem:(NSInteger)item {
    ...
    CATextLayer *title = (CATextLayer *)_titles[_currentSelectedMenudIndex];
    NSInteger currentSelectedMenudRow = [_currentSelectRowArray[_currentSelectedMenudIndex] integerValue];
    title.string = [_dataSource menu:self titleForItemsInRowAtIndexPath:[G100IndexPath indexPathWithCol:_currentSelectedMenudIndex row:currentSelectedMenudRow item:item]];
    [self animateIdicator:_indicators[_currentSelectedMenudIndex] background:_backGroundView tableView:_leftTableView title:_titles[_currentSelectedMenudIndex] forward:NO complecte:^{
        _show = NO;
    }];
...
#pragma mark - 重置 确定操作
- (void)resetFilter {
    if ([_delegate respondsToSelector:@selector(menu:resetFilter:)]) {
        [_delegate menu:self resetFilter:nil];
    }
}
...
- (void)confirmFilter {
    ...
    if ([_delegate respondsToSelector:@selector(menu:confirmFilter:)]) {
        [_delegate menu:self confirmFilter:nil];
    }
}
