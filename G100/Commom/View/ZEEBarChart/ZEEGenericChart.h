...
@class ZEEGenericChart;

/*********************  ZFChartDataSource(ZFChart数据源方法)  *********************/
@protocol ZEEGenericChartDataSource <NSObject>
 ...
 */
- (NSArray *)colorArrayInGenericChart:(ZEEGenericChart *)chart;
 ...
 */
- (CGFloat)axisLineMinValueInGenericChart:(ZEEGenericChart *)chart;
...
@interface ZEEGenericChart : UIView

@property (nonatomic, weak) id<ZEEGenericChartDataSource> dataSource;

/** 主题Label */
@property (nonatomic, strong) UILabel * topicLabel;
/** 图表背景视图 */
@property (nonatomic, strong) UIImageView * gradientBgView;
/** 图表顶部限制线说明文案 */
@property (nonatomic, strong) UILabel * xTopDescLabel;
/** x轴 左侧导向按钮 */
@property (nonatomic, strong) UIButton * leftGuideBtn;
/** y轴单位 */
...
/** y轴(普通图表) 或 x轴(横向图表) 上名称字体大小(默认为10.f) */
@property (nonatomic, strong) UIFont * axisLineNameNormalFont;
/** y轴(普通图表) 或 x轴(横向图表) 上名称字体大小(默认为10.f) */
@property (nonatomic, strong) UIFont * axisLineNameSelectedFont;
...
/** 该属性是否重设坐标轴最小值，默认为NO(不设置，从0开始)，当设置为YES时，则有以下2种情况
    ...
    ①If at the same time to implement the method in ZEEGenericChartDataSource:
        ||- (CGFloat)axisLineMinValueInGenericChart:(ZEEGenericChart *)chart||
