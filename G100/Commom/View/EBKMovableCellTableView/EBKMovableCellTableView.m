...
static NSTimeInterval kEBKMovableCellAnimationTime = 0.25;

@interface EBKMovableCellTableView ()
...
@implementation EBKMovableCellTableView
...
- (void)ebk_addGesture
{
    _gesture = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(ebk_processGesture:)];
    _gesture.minimumPressDuration = _gestureMinimumPressDuration;
    [self addGestureRecognizer:_gesture];
}

- (void)ebk_processGesture:(UILongPressGestureRecognizer *)gesture
{
    switch (gesture.state) {
        case UIGestureRecognizerStateBegan:
        {
            [self ebk_gestureBegan:gesture];
        }
            break;
        case UIGestureRecognizerStateChanged:
        {
            if (!_canEdgeScroll) {
                [self ebk_gestureChanged:gesture];
            }
        }
            break;
        ...
        {
            [self ebk_gestureEndedOrCancelled:gesture];
        }
            break;
        default:
            break;
...
- (void)ebk_gestureBegan:(UILongPressGestureRecognizer *)gesture
{
    ...
    _tempView = [self ebk_snapshotViewWithInputView:cell];
    ...
    //隐藏cell
    cell.hidden = YES;
    [UIView animateWithDuration:kEBKMovableCellAnimationTime animations:^{
        _tempView.center = CGPointMake(_tempView.center.x, point.y);
    }];
}

- (void)ebk_gestureChanged:(UILongPressGestureRecognizer *)gesture
{
    CGPoint point = [gesture locationInView:gesture.view];
    ...
    if (currentIndexPath && ![_selectedIndexPath isEqual:currentIndexPath]) {
        //交换数据源和cell
        [self ebk_updateDataSourceAndCellFromIndexPath:_selectedIndexPath toIndexPath:currentIndexPath];
...
- (void)ebk_gestureEndedOrCancelled:(UILongPressGestureRecognizer *)gesture
{
    if (_canEdgeScroll) {
        [self ebk_stopEdgeScroll];
    }
    //返回交换后的数据源
...
- (UIView *)ebk_snapshotViewWithInputView:(UIView *)inputView
{
...
- (void)ebk_updateDataSourceAndCellFromIndexPath:(NSIndexPath *)fromIndexPath toIndexPath:(NSIndexPath *)toIndexPath
{
...
- (void)ebk_startEdgeScroll
{
    _edgeScrollTimer = [CADisplayLink displayLinkWithTarget:self selector:@selector(ebk_processEdgeScroll)];
    [_edgeScrollTimer addToRunLoop:[NSRunLoop mainRunLoop] forMode:NSRunLoopCommonModes];
}

- (void)ebk_processEdgeScroll
{
    [self ebk_gestureChanged:_gesture];
