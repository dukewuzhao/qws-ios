...
- (void)drawRect:(CGRect)rect
{
    if (self.frame.size.height<216 || self.frame.size.width<320)
        self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, rect.size.width, 216);
    ...
    if (!myPickerView) {
        CGFloat pickerX = (rect.size.width - 320)/2.0;
        myPickerView = [[UIPickerView alloc]initWithFrame:CGRectMake(pickerX, 0, 320, self.frame.size.height)];
...
{
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(point_x + myPickerView.frame.origin.x, 98, 20, 20)];
    label.text = name;
...
- (void)playTheDelegate
{
    ...
    //代理回调
    if ([_delegate respondsToSelector:@selector(uuDatePicker:year:month:day:hour:minute:weekDay:)]) {
