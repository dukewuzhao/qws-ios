...
- (void)setupViews
{
    ...
    _backGroudView.dn_centerY = self.dn_centerY;
    _backGroudView.backgroundColor = [UIColor hexStringToColor:dnSendButtonTintNormalColor];
    _backGroudView.layer.cornerRadius = _backGroudView.dn_height/2;
    ...
    _sendButton.frame = CGRectMake(0, 0, self.dn_width, self.dn_height);
    [_sendButton setTitle:NSLocalizedStringFromTable(@"confirm", @"DNImagePicker", @"确定")
...
- (void)setBadgeValue:(NSString *)badgeValue
{
    ...
    self.badgeValueLabel.frame = CGRectMake(self.badgeValueLabel.dn_left, self.badgeValueLabel.dn_top, (rect.size.width + 9) > 20?(rect.size.width + 9):20, 20);
    self.backGroudView.dn_width = self.badgeValueLabel.dn_width;
    self.backGroudView.dn_height = self.badgeValueLabel.dn_height;
    
    self.sendButton.dn_width = self.badgeValueLabel.dn_width + kSendButtonTextWitdh;
    self.dn_width = self.sendButton.dn_width;
