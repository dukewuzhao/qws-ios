...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.dn_height = 28;
        self.dn_width = CGRectGetWidth([[UIScreen mainScreen] bounds])/2 -20;
        [self fullImageButton];
        //原图按钮关闭
        self.fullImageButton.hidden = YES;
        [self imageSizeLabel];
        [self indicatorView];
        self.selected = NO;
    }
...
- (UIButton *)fullImageButton
{
    if (nil == _fullImageButton) {
        ...
        _fullImageButton.dn_width = [self fullImageButtonWidth];
        _fullImageButton.dn_height = 28;
        ...
        [_fullImageButton setImageEdgeInsets:UIEdgeInsetsMake(6, 0, 6, _fullImageButton.dn_width - buttonImageWidth)];
...
- (UIActivityIndicatorView *)indicatorView
{
    if (nil == _indicatorView) {
        _indicatorView = [[UIActivityIndicatorView alloc] initWithFrame:CGRectMake(self.fullImageButton.dn_width, 2, 26, 26)];
...
- (void)setSelected:(BOOL)selected
{
    if (_selected != selected) {
        ...
        self.fullImageButton.dn_width = [self fullImageButtonWidth];
        [self.fullImageButton setTitleEdgeInsets:UIEdgeInsetsMake(0, buttonPadding-buttonImageWidth, 6, 0)];
        [self.fullImageButton setImageEdgeInsets:UIEdgeInsetsMake(6, 0, 6, self.fullImageButton.dn_width - buttonImageWidth)];
        CGFloat labelWidth = self.dn_width - self.fullImageButton.dn_width;
        self.imageSizeLabel.dn_left = self.fullImageButton.dn_width;
        self.imageSizeLabel.dn_width = labelWidth;
