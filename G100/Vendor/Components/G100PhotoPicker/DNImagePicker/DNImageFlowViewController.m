...
static NSUInteger const kDNImageFlowMaxSeletedNumber = 5;/* 照片最多数量 */
...
- (BOOL)seletedAssets:(ALAsset *)asset
{
    ...
    if (self.selectedAssetsArray.count >= self.maxSeletedNumber - self.pickedCount) {
        NSString *msg = [NSString stringWithFormat:NSLocalizedStringFromTable(@"alertContent", @"DNImagePicker", nil), @(self.maxSeletedNumber)];
...
- (NSInteger)maxSeletedNumber {
    if (!_maxSeletedNumber) {
        _maxSeletedNumber = kDNImageFlowMaxSeletedNumber;
    }
    return _maxSeletedNumber;
}
...
#define kSizeThumbnailCollectionView  ([UIScreen mainScreen].bounds.size.width-10)/4.0
