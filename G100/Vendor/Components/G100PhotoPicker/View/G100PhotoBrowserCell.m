...
#import "G100TapDetectingImageView.h"
#import "G100PhotoBrowserViewController.h"
#import "G100PhotoShowModel.h"
#import <UIImageView+WebCache.h>

@interface G100PhotoBrowserCell () <UIScrollViewDelegate,G100TapDetectingImageViewDelegate>

@property (nonatomic, strong) UIScrollView *zoomingScrollView;
@property (nonatomic, strong) G100TapDetectingImageView *photoImageView;

@end

@implementation G100PhotoBrowserCell
...
- (void)prepareForReuse {
    ...
    self.photoBrowserViewController = nil;
    self.photoModel = nil;
}

- (void)setPhotoModel:(G100PhotoShowModel *)photoModel {
...
- (void)displayImage {
    ...
    if (self.photoModel.data) {
        self.photoImageView.image = [UIImage imageWithData:self.photoModel.data];
        ...
        [self setNeedsLayout];
    }else if (self.photoModel.url) {
        __weak G100PhotoBrowserCell *wself = self;
        [self.photoImageView sd_setImageWithURL:[NSURL URLWithString:self.photoModel.url] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
            // 图片加载完成之后 load一下确保点击事件可以执行
            wself.photoImageView.hidden = NO;
            CGRect photoImageViewFrame;
            photoImageViewFrame.origin = CGPointZero;
            photoImageViewFrame.size = wself.photoImageView.image.size;
            wself.photoImageView.frame = photoImageViewFrame;
            wself.zoomingScrollView.contentSize = photoImageViewFrame.size;
        ...
        self.photoImageView.image = self.photoModel.image;
...
#pragma mark - get
- (G100TapDetectingImageView *)photoImageView
{
    if (nil == _photoImageView) {
        _photoImageView = [[G100TapDetectingImageView alloc] initWithFrame:CGRectZero];
...
- (void)setMaxMinZoomScalesForCurrentBounds {
    ...
    minScale = xScale;
    self.zoomingScrollView.contentSize = CGSizeMake(self.zoomingScrollView.frame.size.width, imageSize.height*(boundsSize.height*imageSize.width)/boundsSize.width);
    ...
    self.zoomingScrollView.contentOffset = CGPointMake(0, 0);
...
- (void)handleSingleTap:(CGPoint)touchPoint {
    [self.photoBrowserViewController performSelector:@selector(toggleControls) withObject:nil afterDelay:0.2];
}

- (void)handleDoubleTap:(CGPoint)touchPoint {
    
    // Cancel any single tap handling
    [NSObject cancelPreviousPerformRequestsWithTarget:self.photoBrowserViewController];
    // Zoom
